...
class TranscriptionService
{
    ...
    private const GEMINI_TIMEOUT_SECONDS = 30;
    ...
    private static bool $missingGeminiKeyWarned = false;
    ...
    public function transcribePhase(DebatePhase $phase): string
    {
        ...
        try {
            ...
            // Gemini failure (already logged inside the method itself).
            $cleaned = $this->cleanTranscriptWithGemini($text);
            if ($cleaned !== null) {
                $phase->update(['speech_text_cleaned' => $cleaned]);

                Log::info('Debate phase transcript cleaned via Gemini', $context + [
    ...
    /**
     * Best-effort Gemini cleanup pass over a raw Whisper transcript — removes
     ...
     *
     * NOTE (deliberate deviation from the originally described `?key=` URL
     * query parameter): the API key is sent via the `x-goog-api-key` header
     * instead, which Google's Generative Language API supports identically.
     * Putting a secret in a URL risks it being echoed into cURL connection-
     * exception messages or incidental logging; the header form has no such
     * risk and changes nothing about the actual request being made.
     */
    private function cleanTranscriptWithGemini(string $rawText): ?string
    {
        $apiKey = (string) config('services.gemini.api_key');

        if ($apiKey === '') {
            if (! self::$missingGeminiKeyWarned) {
                Log::warning('Gemini transcript cleanup skipped — GEMINI_API_KEY is not configured');
                self::$missingGeminiKeyWarned = true;
            }
        ...
        $prompt = <<<PROMPT
        This is a raw speech-to-text transcript of an Arabic debate speech, generated by an automatic speech recognition system. It may contain repeated hallucinated phrases (like phrases about subscribing to a channel, or repeated words like "thank you"), stray non-Arabic words or gibberish that don't belong, and minor recognition errors. Clean this transcript: remove hallucinated or repeated filler phrases and non-Arabic noise, fix obvious word-level transcription errors where you are confident, but preserve the speaker's actual meaning and content as faithfully as possible. Do not add any information that wasn't in the original. Do not summarize or shorten the real content. Return ONLY the cleaned Arabic text, with no preamble, no explanation, no markdown — just the cleaned transcript itself.
        ...
        try {
            $response = Http::timeout(self::GEMINI_TIMEOUT_SECONDS)
            ...
            if (! $response->successful()) {
                Log::error('Gemini transcript cleanup failed — non-2xx response', [
            ...
            if (! is_string($cleaned) || trim($cleaned) === '') {
                Log::error('Gemini transcript cleanup failed — unexpected response shape', [
                    'body' => $response->body(),
                ]);
            ...
            Log::error('Gemini transcript cleanup failed — exception', [
                'error' => $e->getMessage(),
            ]);
